...
export default function ModulerClient({ initialProjects }: { initialProjects: Project[] }) {
  const t = useTranslations("modularPage");
  ...
  const { title, description } = useMemo(() => {
    switch (sub) {
      case "bungalow":
      case "tiny-house":
        return { title: t("bungalowTitle"), description: t("bungalowDesc") };
      case "ofis":
      case "moduler-ev":
        return { title: t("officeTitle"), description: t("officeDesc") };
      case "sifir-atik":
        return { title: t("zeroWasteTitle"), description: t("zeroWasteDesc") };
      default:
  ...
  const breadcrumbItems = [
    ...
    { name: tNav("modular"), href: "/modular" as const },
    ...(sub ? [{ name: title, href: { pathname: "/modular" as const, query: { category: sub } } }] : []),
  ];
