...
export default function KonteynerClient({ initialProjects }: { initialProjects: Project[] }) {
  const t = useTranslations("containerPage");
  ...
  const { title, description } = useMemo(() => {
    switch (sub) {
      case "ev":
      case "ofis":
        return { title: t("homeOfficeTitle"), description: t("homeOfficeDesc") };
      case "guvenlik-kulubesi":
        return { title: t("securityTitle"), description: t("securityDesc") };
      case "wc-dus-kabini":
        return { title: t("wcTitle"), description: t("wcDesc") };
      default:
  ...
  const breadcrumbItems = [
    ...
    { name: tNav("container"), href: "/container" as const },
    ...(sub ? [{ name: title, href: { pathname: "/container" as const, query: { category: sub } } }] : []),
  ];
  ...
  return (
    <main className="min-h-screen">
              ...
              <motion.div key={p.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.05 }} viewport={{ once: true, margin: "-50px" }} className="group">
                <motion.div className="relative bg-white rounded-2xl shadow-xl overflow-hidden cursor-pointer" whileHover={{ scale: 1.02, y: -5 }} transition={{ duration: 0.2 }}>
                  ...
                  <motion.div className="hidden sm:flex absolute inset-0 bg-black/60 items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <Button size="lg" onClick={() => { setSelectedProject(p); setSelectedImageIdx(0); }} className="bg-accent hover:bg-accent/80 text-white px-6 py-3 text-base font-medium rounded-2xl shadow-xl transition-all duration-300 hover:shadow-2xl hover:scale-105">
                      {tCommon("viewDetails")}
      ...
      {selectedProject && (
        <GalleryModal title={selectedProject.title} specs={selectedProject.specs} images={selectedProject.images} mainImage={selectedProject.image} selectedIdx={selectedImageIdx} onIndexChange={setSelectedImageIdx} onClose={() => setSelectedProject(null)} />
      )}
      <Footer />
    </main>
  );
